...
saveRDS(pred.stanford, file = "singleR_stanford.rds")
pred.stanford
...
#### color schemes (for reproducible external plots) ----
...
## now we can learn the temporal order (order)
# this calls up a shiny app, choose the ROOT NODE
stanford.cds <- order_cells(stanford.cds)
# here i chose node 2 (SMC) and node 5 (monocyte)
...
write.csv(pr_graph_test_res, file = "stanford_moransI_in_SMC_subcluster.csv")
...
plot_cells(stanford.cds_subset,
           ...
           show_trajectory_graph=F)
dev.off()
#### Subset Trajectory & analysis of SC/OS----
stanford.cds_subset2 <- choose_cells(stanford.cds) # calls up shiny app

plot_cells(stanford.cds_subset2,
           color_cells_by = "pseudotime",
           show_trajectory_graph = T,
           trajectory_graph_segment_size = 1,
           label_leaves=F, # this gives a little node label (outcome)
...
## now lets do the subsets
pr_graph_test_res.sub <- graph_test(stanford.cds_subset2, neighbor_graph="principal_graph", cores=2)
...
pdf("Figure_images/monocle3_genesoverpseudotime_seuratpartition.pdf", width=7, height=6)
plot_cells(stanford.cds_subset2, 
           genes=c("TNFRSF1B", "CASP9", "PARK7"), # this is faceting by the genes that are DE
           show_trajectory_graph=FALSE, 
           label_cell_groups=FALSE)
